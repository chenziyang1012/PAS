  ...
  <div>
    <el-card>
      ...
      <el-table :data="list" v-loading="loading" @selection-change="selected=$event">
        ...
        <el-table-column prop="done_at" label="完成时间" width="150">
          <template #default="{row}">{{ (row.done_at || row.updated_at)?.slice(0,19).replace('T',' ') }}</template>
        </el-table-column>
...
async function doBulkImport() {
  ...
  try {
    const res: any = await productApi.bulkCreate(urls, 'done')
